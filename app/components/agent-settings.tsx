    ...
    <div className="space-y-8 pb-20">
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between px-1">
          <div className="flex items-center gap-2">
            <Sparkles className="w-5 h-5 text-[#E2725B]" />
            <h3 className="font-bold text-[#FAFAFA]">Quick Start Templates</h3>
          </div>
          <span className="text-xs font-semibold text-[#A1A1AA] uppercase tracking-widest">Select to pre-fill</span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {AGENT_TEMPLATES.map((template) => (
            ...
            >
              ...
              <h4 className="font-bold text-[#FAFAFA] text-sm mb-1">{template.name}</h4>
      ...
      <section className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm space-y-6">
        <div className="flex items-center gap-3 mb-2">
          <div className="w-10 h-10 bg-[#E2725B]/10 rounded-xl flex items-center justify-center text-[#E2725B]">
            <User className="w-5 h-5" />
          </div>
          <div>
            <h3 className="text-lg font-bold">Identity & Purpose</h3>
            <p className="text-sm text-[#A1A1AA]">Define how your agent identifies and its primary mission.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="text-sm font-bold text-gray-700 flex items-center gap-2">
              Agent Name
              <Info className="w-3.5 h-3.5 text-[#A1A1AA] cursor-help" />
            </label>
            <input 
              type="text" 
              placeholder="e.g. Alex"
              className={`w-full px-4 py-3 bg-gray-50 border rounded-xl focus:ring-2 focus:ring-[#E2725B] outline-none transition-all ${errors.name ? 'border-red-300' : 'border-white/10'}`}
          ...
          <div className="space-y-2">
            ...
            <input 
              ...
              className="w-full px-4 py-3 bg-gray-50 border border-white/10 rounded-xl focus:ring-2 focus:ring-[#E2725B] outline-none transition-all"
        ...
        <div className="space-y-2">
          <label className="text-sm font-bold text-gray-700 flex items-center justify-between">
            Master Instructions
            <span className="text-[10px] text-[#A1A1AA] font-mono">{localConfig.purpose?.length || 0}/1000</span>
          </label>
          <textarea 
            rows={5}
            placeholder="Tell your agent exactly what to do and how to handle tricky situations..."
            className={`w-full px-4 py-3 bg-gray-50 border rounded-xl focus:ring-2 focus:ring-[#E2725B] outline-none resize-none transition-all ${errors.purpose ? 'border-red-300' : 'border-white/10'}`}
            value={localConfig.purpose}
            onChange={(e) => handleInputChange("purpose", e.target.value)}
          />
          {errors.purpose && <p className="text-xs text-red-500 font-medium">{errors.purpose}</p>}
          <p className="text-[11px] text-[#A1A1AA] italic">Pro tip: Mention specific constraints like "never give legal advice".</p>
        </div>
      </section>
      ...
      <section className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm space-y-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-sm text-[#A1A1AA]">Fine-tune the personality and communication style.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              {TONES.map(tone => (
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all cursor-pointer ${
                    ...
                    : 'bg-gray-50 text-[#A1A1AA] hover:bg-white/[0.06]'
                  }`}
                >
                  {tone}
                </button>
          ...
          <div className="space-y-4">
            ...
            <div className="relative">
              <select 
                className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-white/10 rounded-xl focus:ring-2 focus:ring-[#E2725B] outline-none appearance-none transition-all"
      ...
      <section className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm space-y-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-sm text-[#A1A1AA]">Control safety guards and availability windows.</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-white rounded-lg shadow-sm">
                <Shield className="w-4 h-4 text-[#E2725B]" />
              </div>
              <div>
                <p className="text-sm font-bold">Strict Safety Filtering</p>
                <p className="text-xs text-[#A1A1AA]">Auto-reject inappropriate or risky prompts.</p>
              </div>
            </div>
            <button 
              onClick={() => handleInputChange("safetyFilter", !localConfig.safetyFilter)}
              className={`w-12 h-6 rounded-full transition-all relative ${localConfig.safetyFilter !== false ? 'bg-[#E2725B]' : 'bg-gray-200'} cursor-pointer`}
            >
          ...
          <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100 space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold">Action Approval Mode</p>
                <p className="text-xs text-[#A1A1AA]">How your agent handles actions like sending emails or creating events.</p>
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mt-2">
              {([
                { id: "confirm", label: "Draft → Confirm", desc: "Shows you a preview before acting", color: "green" },
                { id: "notify", label: "Execute + Notify", desc: "Acts immediately, tells you after", color: "amber" },
                { id: "auto", label: "Full Auto", desc: "Acts proactively without asking", color: "red" },
              ] as const).map((mode) => (
                <button
                  key={mode.id}
                  onClick={() => handleInputChange("approvalMode", mode.id)}
                  className={`p-3 rounded-xl text-left transition-all cursor-pointer border ${
                    (localConfig.approvalMode || "confirm") === mode.id
                      ? "bg-[#E2725B] text-white border-indigo-600 shadow-md shadow-indigo-100"
                      : "bg-white text-gray-700 border-white/10 hover:border-indigo-200"
                  }`}
                >
                  <p className={`text-xs font-bold ${(localConfig.approvalMode || "confirm") === mode.id ? "text-white" : "text-[#FAFAFA]"}`}>
                    {mode.label}
                  </p>
                  <p className={`text-[10px] mt-0.5 ${(localConfig.approvalMode || "confirm") === mode.id ? "text-white/80" : "text-[#A1A1AA]"}`}>
                    {mode.desc}
                  </p>
                </button>
              ))}
          ...
          <div className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-[#A1A1AA]">Only reply during your set business hours.</p>
              </div>
            </div>
            <button 
              onClick={() => handleInputChange("officeHours", !localConfig.officeHours)}
              className={`w-12 h-6 rounded-full transition-all relative ${localConfig.officeHours ? 'bg-[#E2725B]' : 'bg-gray-200'} cursor-pointer`}
            >
      ...
      <section className="bg-red-50/50 rounded-3xl p-8 border border-red-100 space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 bg-white rounded-2xl border border-red-100 shadow-sm">
          <div>
            <p className="text-sm font-bold text-[#FAFAFA]">Reset Agent Memory</p>
            <p className="text-xs text-[#A1A1AA]">Clear all past conversation history for this agent.</p>
          </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 bg-white rounded-2xl border border-red-100 shadow-sm">
          <div>
            <p className="text-sm font-bold text-[#FAFAFA]">Deactivate Agent</p>
            <p className="text-xs text-[#A1A1AA]">This will immediately disconnect all active integrations.</p>
          </div>
      ...
      {JSON.stringify(config) !== JSON.stringify(localConfig) && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-[#111111] text-white px-6 py-3 rounded-2xl shadow-2xl flex items-center gap-4 z-50 animate-in fade-in slide-in-from-bottom-4">
          <span className="text-sm font-medium">You have unsaved changes</span>
          <div className="w-px h-4 bg-white/20" />
          <div className="flex gap-3">
            <button 
              onClick={() => setLocalConfig(config)}
              className="text-xs font-bold text-[#A1A1AA] hover:text-white transition-colors cursor-pointer"
            >
              Discard
            </button>
            <button 
              onClick={() => toast.info("Please use the Save button at the top right")}
              className="text-xs font-bold text-[#E2725B] hover:text-[#E2725B] transition-colors cursor-pointer"
            >
              Save Changes
            </button>
          </div>
