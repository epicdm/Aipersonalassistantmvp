    ...
    return (
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-zinc-100">Call Routing</h3>
            <p className="text-xs text-zinc-500">Create an agent first to configure call routing</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-5">
      ...
      {config.enabled && (
        <div className="space-y-4 pt-1">
          {/* Voice Agent Toggle */}
          <div className="flex items-start gap-3 p-4 bg-zinc-800/50 border border-zinc-700/50 rounded-xl">
            <Bot className="w-5 h-5 text-indigo-400 mt-0.5 shrink-0" />
            <div className="flex-1 space-y-1">
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium text-zinc-200">AI Voice Agent</p>
                <button
                  onClick={() => setConfig((c) => ({ ...c, voiceAgentEnabled: !c.voiceAgentEnabled }))}
                  className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${
                    config.voiceAgentEnabled ? "bg-indigo-600" : "bg-zinc-700"
                  }`}
                >
                  <span
          ...
          <div>
            <label className="block text-xs font-medium text-zinc-400 mb-1.5">
              PBX Extension{" "}
              <span className="text-zinc-600 font-normal">(optional — override AI routing)</span>
            </label>
            <input
              type="text"
              ...
              placeholder="e.g. 201"
              className="w-full px-4 py-2.5 bg-zinc-800 border border-zinc-700 rounded-xl text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors"
            />
          ...
          <div>
            <label className="block text-xs font-medium text-zinc-400 mb-1.5">
              ...
              <span className="text-zinc-600 font-normal">(optional — human handoff)</span>
            </label>
            <input
              type="tel"
          ...
          <div>
            <label className="block text-xs font-medium text-zinc-400 mb-1.5 flex items-center gap-1.5">
              ...
              Agent Instructions{" "}
              <span className="text-zinc-600 font-normal">(shared by text + voice)</span>
            </label>
            <textarea
              rows={4}
              ...
              className="w-full px-4 py-3 bg-zinc-800 border border-zinc-700 rounded-xl text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors resize-none"
            />
            <p className="text-xs text-zinc-600 mt-1">
          ...
          {/* Routing summary */}
          <div className="flex items-center gap-2 text-xs text-zinc-500 bg-zinc-800/50 rounded-xl px-4 py-3">
            <ArrowRight className="w-3.5 h-3.5 text-indigo-400 shrink-0" />
            <span>
              Inbound call →{" "}
              {config.voiceAgentEnabled ? (
                <span className="text-indigo-400 font-medium">AI Voice Agent</span>
              ) : (
                <span className="text-zinc-400">skips AI voice</span>
              )}
              {config.sipExtension && (
      ...
      <button
        onClick={handleSave}
        disabled={saving}
        className="flex items-center justify-center gap-2 w-full px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-xl text-sm font-semibold transition-all"
      >
        {saving ? (
