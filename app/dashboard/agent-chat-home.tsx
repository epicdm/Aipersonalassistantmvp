...
const PERSONA_COLORS: Record<string, { bg: string; glow: string; border: string; accent: string }> = {
  ...
  enthusiastic: { bg: "from-[#D4A373] to-[#E2725B]", glow: "shadow-violet-500/20", border: "border-[#D4A373]/30", accent: "text-[#D4A373]" },
  empathetic: { bg: "from-rose-600 to-pink-700", glow: "shadow-rose-500/20", border: "border-rose-500/30", accent: "text-rose-400" },
};
const DEFAULT_COLORS = { bg: "from-[#E2725B] to-[#D4A373]", glow: "shadow-[#E2725B]/20", border: "border-[#E2725B]/30", accent: "text-[#E2725B]" };
const NAV_ITEMS = [
...
function TypedBubble({ text, from, align, delay = 0, color = "bg-[#1A1A1A]" }: { text: string; from: string; align: "left" | "right"; delay?: number; color?: string }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.3 }}
      ...
      <div className={`max-w-[85%] ${color} rounded-2xl ${align === "right" ? "rounded-br-sm" : "rounded-bl-sm"} px-3.5 py-2.5`}>
        ...
        <p className="text-[9px] text-[#A1A1AA] mt-1 text-right">{from}</p>
      </div>
    </motion.div>
  );
}
...
function WhatsAppSim({ agentName }: { agentName: string }) {
  return (
    <div className="bg-[#1A1A1A]/40 border border-white/[0.05] rounded-xl overflow-hidden">
      <div className="bg-green-900/40 px-3 py-2 flex items-center gap-2 border-b border-white/[0.05]">
        <MessageSquare className="w-3.5 h-3.5 text-green-400" />
        <span className="text-[11px] font-bold text-green-400">WhatsApp</span>
        <span className="text-[10px] text-[#A1A1AA] ml-auto">Customer → You</span>
      </div>
      ...
      <div className="bg-green-900/20 px-3 py-2 border-t border-white/[0.05]">
...
function PhoneSim({ agentName }: { agentName: string }) {
  ...
  return (
    <div className="bg-[#1A1A1A]/40 border border-white/[0.05] rounded-xl overflow-hidden">
      <div className="bg-blue-900/40 px-3 py-2 flex items-center gap-2 border-b border-white/[0.05]">
        <Phone className="w-3.5 h-3.5 text-blue-400" />
        <span className="text-[11px] font-bold text-blue-400">Incoming Call</span>
        <span className="text-[10px] text-[#A1A1AA] ml-auto">+1 (767) 555-1234</span>
      </div>
        ...
        {stage >= 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-[#A1A1AA]">Unknown number calling</p>
            </div>
          </motion.div>
        )}
        {stage >= 1 && (
          <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-[#A1A1AA] italic">&quot;Hi! Thanks for calling. How can I help you today?&quot;</p>
            </div>
          </motion.div>
        )}
        {stage >= 2 && (
          <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] text-[#A1A1AA] italic">&quot;I&apos;m having trouble with my internet. It keeps dropping.&quot;</p>
            </div>
          </motion.div>
        )}
        {stage >= 3 && (
          <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] text-[#A1A1AA] italic">&quot;I&apos;m sorry to hear that. Let me check your account... I see some signal issues. I&apos;ll schedule a technician for tomorrow. Does 10 AM work?&quot;</p>
            </div>
          </motion.div>
        )}
        {stage >= 4 && (
          <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-2 flex items-center gap-2">
            ...
            <p className="text-[10px] text-[#A1A1AA]"><strong className="text-blue-400">You get notified:</strong> &quot;Call handled — technician booked for tomorrow 10 AM for internet issue.&quot;</p>
          </motion.div>
        )}
      </div>
      <div className="bg-blue-900/20 px-3 py-2 border-t border-white/[0.05]">
...
function ContactsSim({ agentName }: { agentName: string }) {
  ...
  return (
    <div className="bg-[#1A1A1A]/40 border border-white/[0.05] rounded-xl overflow-hidden">
      <div className="bg-[#E2725B]/10 px-3 py-2 flex items-center gap-2 border-b border-white/[0.05]">
        <Users className="w-3.5 h-3.5 text-[#D4A373]" />
        <span className="text-[11px] font-bold text-[#D4A373]">Outreach</span>
        <span className="text-[10px] text-[#A1A1AA] ml-auto">3 contacts</span>
      </div>
      <div className="p-3 space-y-2 min-h-[120px]">
        {stage >= 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-[10px] text-[#A1A1AA] mb-2">
        ...
        {[
          ...
          <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
            ...
            <div className="w-7 h-7 bg-[#D4A373]/20 rounded-full flex items-center justify-center text-[10px] font-bold text-[#D4A373] shrink-0">
              {c.name[0]}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-[11px] font-bold text-white">{c.name}</p>
              <p className="text-[10px] text-[#A1A1AA] truncate">{c.msg}</p>
            </div>
      ...
      <div className="bg-[#D4A373]/08 px-3 py-2 border-t border-white/[0.05]">
        <p className="text-[10px] text-[#D4A373]/80 text-center">👥 One command → {agentName} reaches everyone</p>
      </div>
    </div>
  );
}
...
function KnowledgeSim({ agentName }: { agentName: string }) {
  ...
  return (
    <div className="bg-[#1A1A1A]/40 border border-white/[0.05] rounded-xl overflow-hidden">
      <div className="bg-amber-900/40 px-3 py-2 flex items-center gap-2 border-b border-white/[0.05]">
        ...
        {stage >= 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-2 items-start">
            ...
            <div className="bg-gray-700/50 rounded-xl rounded-bl-sm px-3 py-2">
              <p className="text-[11px] text-[#FAFAFA]/80">&quot;What are your internet plans and pricing?&quot;</p>
            </div>
          </motion.div>
        )}
        {stage >= 1 && (
        ...
        {stage >= 2 && (
          <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="flex gap-2 items-start justify-end">
            <div className="bg-amber-700/20 border border-amber-600/20 rounded-xl rounded-br-sm px-3 py-2 max-w-[85%]">
              <p className="text-[11px] text-[#FAFAFA]">We have three plans:</p>
              <p className="text-[11px] text-[#FAFAFA]/80 mt-1">📶 <strong>Basic</strong> — $49/mo, 50 Mbps</p>
              <p className="text-[11px] text-[#FAFAFA]/80">🚀 <strong>Pro</strong> — $79/mo, 100 Mbps</p>
              <p className="text-[11px] text-[#FAFAFA]/80">⚡ <strong>Ultra</strong> — $129/mo, 300 Mbps</p>
              <p className="text-[11px] text-[#FAFAFA]/80 mt-1">All plans include free installation. Want me to sign you up?</p>
              <p className="text-[9px] text-[#A1A1AA] mt-1 text-right">from pricing-2026.pdf</p>
            </div>
          </motion.div>
        )}
        {stage >= 3 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-1.5 px-2">
            ...
            <p className="text-[10px] text-[#A1A1AA]">Answer sourced from <strong className="text-amber-400">your docs</strong> — not made up</p>
          </motion.div>
        )}
      </div>
      <div className="bg-amber-900/20 px-3 py-2 border-t border-white/[0.05]">
...
function IntegrationsSim({ agentName }: { agentName: string }) {
  ...
  return (
    <div className="bg-[#1A1A1A]/40 border border-white/[0.05] rounded-xl overflow-hidden">
      <div className="bg-yellow-900/40 px-3 py-2 flex items-center gap-2 border-b border-white/[0.05]">
        <Zap className="w-3.5 h-3.5 text-yellow-400" />
        <span className="text-[11px] font-bold text-yellow-400">Connected Tools</span>
      </div>
      <div className="p-3 space-y-2.5 min-h-[120px]">
        {stage >= 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-[10px] text-[#A1A1AA]">
        ...
        {[
          ...
          <motion.div key={i} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }}
            ...
            <p className="text-[11px] text-[#FAFAFA]/80">{item.text}</p>
          </motion.div>
        ))}
        {stage >= 4 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center pt-1">
      ...
      <div className="bg-yellow-900/20 px-3 py-2 border-t border-white/[0.05]">
...
function CapabilityWizard({ id, agentName, onDone, contacts }: { id: string; agentName: string; onDone: () => void; contacts: any[] }) {
    ...
    <div className="space-y-4">
      ...
      {phase === "setup" && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
          <label className="text-xs font-bold text-[#A1A1AA] block">Your WhatsApp phone number</label>
          <div className="relative">
            <Phone className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[#A1A1AA]" />
            <input type="tel" placeholder="+1 767 295 8382" value={phone} onChange={e => setPhone(e.target.value)} onKeyDown={e => e.key === "Enter" && saveWhatsApp()}
              className="w-full pl-10 pr-4 py-3 bg-[#1A1A1A] border border-white/10 rounded-xl text-white placeholder-[#A1A1AA] focus:border-green-500 focus:outline-none text-sm" />
          </div>
          <p className="text-[10px] text-[#A1A1AA]">Include country code. {agentName} will message from this number.</p>
          <div className="flex gap-2">
            <button onClick={() => setPhase("sim")} className="px-4 py-3 text-sm text-[#A1A1AA] hover:text-white rounded-xl cursor-pointer">← Back</button>
      ...
      {phase === "done" && (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-3 space-y-2">
          ...
          <p className="text-xs text-[#A1A1AA]">Just like the demo — {agentName} will handle messages for you 24/7.</p>
        </motion.div>
      )}
    </div>
  );
    ...
    <div className="space-y-4">
      ...
      {phase === "setup" && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
          <div className="bg-white/[0.04] border border-white/10 rounded-xl p-4 text-center">
            <p className="text-sm text-[#FAFAFA]/80">We&apos;ll assign a phone number to <strong className="text-white">{agentName}</strong>.</p>
            <p className="text-xs text-[#A1A1AA] mt-2">People call → {agentName} answers. Just like the demo.</p>
          </div>
          <div className="flex gap-2">
            <button onClick={() => setPhase("sim")} className="px-4 py-3 text-sm text-[#A1A1AA] hover:text-white rounded-xl cursor-pointer">← Back</button>
      ...
      {phase === "done" && (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-3 space-y-3">
          ...
          <p className="text-[10px] text-[#A1A1AA]">Try from your phone — it works just like the demo above!</p>
        </motion.div>
      )}
    </div>
  );
    ...
    <div className="space-y-4">
      {phase === "sim" && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
          ...
          <button onClick={() => setPhase("setup")} className="w-full py-3 bg-[#E2725B] text-white rounded-xl font-bold text-sm hover:bg-[#F48B76] transition-all cursor-pointer">Add my contacts →</button>
        </motion.div>
      )}
      {phase === "setup" && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
          <label className="text-xs font-bold text-[#A1A1AA] block">Type name + phone or email</label>
          <div className="flex gap-2">
            <input type="text" placeholder='e.g. "Sarah Chen +17675551234"' value={quickInput} onChange={e => setQuickInput(e.target.value)} onKeyDown={e => e.key === "Enter" && addContact()}
              className="flex-1 px-4 py-3 bg-[#1A1A1A] border border-white/10 rounded-xl text-white placeholder-[#A1A1AA] focus:border-violet-500 focus:outline-none text-sm" />
            <button onClick={addContact} disabled={!quickInput.trim()} className="px-5 py-3 bg-[#E2725B] text-white rounded-xl font-bold text-sm hover:bg-[#F48B76] disabled:opacity-30 cursor-pointer">Add</button>
          </div>
          {added.map((name, i) => (
            <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-2 text-xs py-1.5 px-3 bg-white/[0.04] rounded-lg">
              <CheckCircle className="w-3.5 h-3.5 text-green-400" /> <strong className="text-white">{name}</strong> added
            </motion.div>
          ))}
          <div className="flex items-center justify-between">
            <button onClick={() => setPhase("sim")} className="text-xs text-[#A1A1AA] hover:text-white cursor-pointer">← Back</button>
            <button onClick={() => router.push("/contacts")} className="text-xs text-[#D4A373] hover:text-[#D4A373] cursor-pointer flex items-center gap-1">Import CSV / bulk <ArrowRight className="w-3 h-3" /></button>
          </div>
          {added.length >= 1 && (
            <p className="text-[10px] text-[#A1A1AA] text-center">✅ {agentName} can now reach {added.join(", ")} — just like the demo!</p>
          )}
        </motion.div>
      )}
    </div>
  ...
  if (id === "knowledge") return (
    <div className="space-y-4">
      ...
      <p className="text-[10px] text-[#A1A1AA] text-center">PDF, TXT, DOCX — your pricing, FAQ, product info, anything.</p>
    </div>
  );
...
export default function AgentChatHome() {
  ...
  const CAPABILITIES = [
    ...
    { id: "contacts", icon: Users, title: "Reach out to people", desc: "Message everyone at once with one command", color: "text-[#D4A373] bg-[#D4A373]/10", accent: "border-[#D4A373]/30" },
  ...
  return (
    <div className="h-screen bg-[#050505] text-white flex flex-col relative overflow-hidden">
      ...
      <div className="relative z-10 flex items-center justify-between px-5 py-3 border-b border-white/[0.05]">
        <div className="flex items-center gap-3 cursor-pointer" onClick={() => { setView(completedCards.length >= 2 ? "office" : "welcome"); setExpandedCard(null); setMessages([]); }}>
          ...
          <div><h1 className="font-bold text-sm">{agentName}</h1><div className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 bg-green-500 rounded-full" /><span className="text-[10px] text-[#A1A1AA]">Online</span></div></div>
        </div>
        <div className="flex items-center gap-1">
          {NAV_ITEMS.slice(0, 4).map(item => (
            <button key={item.href} onClick={() => router.push(item.href)} className="p-2.5 text-[#A1A1AA] hover:text-white hover:bg-[#1A1A1A] rounded-xl transition-all cursor-pointer" title={item.label}><item.icon className="w-4 h-4" /></button>
          ))}
          <button onClick={() => setShowNav(!showNav)} className="p-2.5 text-[#A1A1AA] hover:text-white hover:bg-[#1A1A1A] rounded-xl transition-all cursor-pointer"><MoreHorizontal className="w-4 h-4" /></button>
        </div>
      </div>

      <AnimatePresence>
        {showNav && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute top-14 right-4 z-50 bg-[#111111] border border-white/[0.07] rounded-2xl shadow-2xl p-2 min-w-[180px]">
            {NAV_ITEMS.map(item => (
              <button key={item.href} onClick={() => { router.push(item.href); setShowNav(false); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-[#A1A1AA] hover:text-white hover:bg-[#1A1A1A] rounded-xl transition-all cursor-pointer"><item.icon className="w-4 h-4" /> {item.label}</button>
            ))}
            <div className="border-t border-white/[0.07] mt-1 pt-1">
              <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-red-400 hover:text-red-300 hover:bg-[#1A1A1A] rounded-xl transition-all cursor-pointer"><LogOut className="w-4 h-4" /> Logout</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {view === "office" && messages.length === 0 ? (
          ...
          <div className="max-w-xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-6 pt-2">
              ...
              <p className="text-[#A1A1AA] text-xs mt-1">Tap any card to see me in action ↓</p>
            </motion.div>
              ...
              {CAPABILITIES.map((cap, i) => {
                ...
                return (
                  <motion.div key={cap.id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 + i * 0.06 }}
                    className={`bg-[#111111]/80 border rounded-2xl transition-all overflow-hidden ${isOpen ? cap.accent : "border-white/[0.07] hover:border-white/10"}`}>
                    <button onClick={() => toggleCard(cap.id)} className="w-full flex items-center gap-3 p-4 text-left cursor-pointer">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className={`font-bold text-sm ${done && !isOpen ? "text-[#A1A1AA]" : "text-white"}`}>{cap.title}</h3>
                        {!isOpen && <p className="text-xs text-[#A1A1AA] mt-0.5">{cap.desc}</p>}
                      </div>
                      {isOpen ? <ChevronDown className="w-4 h-4 text-[#A1A1AA] shrink-0" /> : <ChevronRight className="w-4 h-4 text-[#A1A1AA] shrink-0" />}
                    </button>
                    <AnimatePresence>
                      {isOpen && (
            ...
            <motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} onClick={() => router.push("/settings")}
              className="w-full mt-3 p-3 bg-[#0d0d0d] border border-white/[0.07] rounded-xl flex items-center gap-3 hover:border-white/10 transition-all cursor-pointer group">
              <Settings className="w-5 h-5 text-[#A1A1AA] group-hover:text-white transition-colors" />
              <span className="text-xs text-[#A1A1AA] group-hover:text-[#FAFAFA]/80 transition-colors">Change my name, personality, or how much freedom I get</span>
              <ChevronRight className="w-4 h-4 text-gray-700 ml-auto group-hover:text-white transition-colors" />
            </motion.button>

            <div className="mt-6 mb-3"><div className="flex items-center gap-4"><div className="flex-1 h-px bg-[#1A1A1A]" /><span className="text-xs text-[#A1A1AA]">or just ask me anything</span><div className="flex-1 h-px bg-[#1A1A1A]" /></div></div>
            <div className="flex flex-wrap gap-2 justify-center pb-4">
              {["What can you do?", "Message someone for me", "What's on my schedule?", "Draft an email"].map(q => (
                <button key={q} onClick={() => send(q)} className="px-4 py-2 bg-[#111111] border border-white/[0.07] rounded-full text-xs text-[#A1A1AA] hover:text-white hover:border-gray-600 transition-all cursor-pointer">{q}</button>
              ))}
            </div>
          </div>
        ) : (
            ...
            <AnimatePresence initial={false}>
              {messages.map(msg => (
                <motion.div key={msg.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.15 }} className={`flex gap-2.5 ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                  ...
                  <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 ${msg.role === "user" ? `bg-gradient-to-br ${colors.bg} text-white rounded-br-md` : "bg-[#111111] border border-white/[0.07] text-[#FAFAFA] rounded-bl-md"}`}>
                    <p className="text-sm whitespace-pre-wrap leading-relaxed">{msg.text}</p>
                    <p className={`text-[10px] mt-1 ${msg.role === "user" ? "text-white/40" : "text-[#A1A1AA]"}`}>{msg.timestamp.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p>
                  </div>
                  {msg.role === "user" && <div className="w-7 h-7 bg-[#1A1A1A] rounded-lg shrink-0 flex items-center justify-center text-[#A1A1AA] mt-1"><User className="w-3.5 h-3.5" /></div>}
                </motion.div>
              ))}
            </AnimatePresence>
            {typing && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-2.5">
                ...
                <div className="bg-[#111111] border border-white/[0.07] rounded-2xl rounded-bl-md px-4 py-3"><div className="flex gap-1.5"><span className="w-1.5 h-1.5 bg-gray-500 rounded-full animate-bounce" /><span className="w-1.5 h-1.5 bg-gray-500 rounded-full animate-bounce [animation-delay:0.15s]" /><span className="w-1.5 h-1.5 bg-gray-500 rounded-full animate-bounce [animation-delay:0.3s]" /></div></div>
              </motion.div>
            )}
          </div>
        )}
      ...
      <div className="relative z-10 px-4 md:px-6 pb-5 pt-2">
        <form onSubmit={e => { e.preventDefault(); send(input); }} className="max-w-2xl mx-auto">
          <div className={`flex items-center gap-2 bg-[#111111] border ${colors.border} rounded-2xl p-2 pr-3 transition-all focus-within:border-opacity-60`}>
            <button type="button" className="p-2 text-[#A1A1AA] hover:text-[#A1A1AA] rounded-lg transition-all cursor-pointer"><Paperclip className="w-4 h-4" /></button>
            <input ref={inputRef} type="text" placeholder={`Talk to ${agentName}...`} className="flex-1 bg-transparent border-none outline-none text-sm py-1.5 text-[#FAFAFA] placeholder:text-[#A1A1AA]" value={input} onChange={e => setInput(e.target.value)} />
