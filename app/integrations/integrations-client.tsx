...
const INTEGRATIONS = [
  ...
  { name: "Notion", desc: "Connect your Notion workspace", icon: Database, color: "text-gray-300 bg-gray-500/10", status: "coming" },
...
export default function IntegrationsClient() {
  return (
    <DarkShell title="Integrations">
      ...
      <p className="text-gray-500 text-sm mb-8">Connect services to expand your agent's abilities</p>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {INTEGRATIONS.map((int) => (
          <div key={int.name} className="bg-gray-900 border border-gray-800 rounded-2xl p-5 flex items-start gap-4 hover:border-gray-700 transition-all">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2">
                ...
                {int.status === "coming" && <span className="text-[9px] px-2 py-0.5 bg-gray-800 text-gray-500 rounded-full font-bold">COMING SOON</span>}
              </div>
              <p className="text-xs text-gray-500 mt-0.5">{int.desc}</p>
            </div>
            {int.status === "available" && (
              <button className="px-4 py-2 bg-indigo-600/10 text-indigo-400 rounded-lg text-xs font-bold hover:bg-indigo-600/20 transition-all cursor-pointer">Connect</button>
            )}
          </div>
        ))}
      </div>
